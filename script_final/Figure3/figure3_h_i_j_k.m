...
pT=[];pT2=[];  nn=0;nn1=0;
CHANNEL=1;sw_t=3;
i_motion_threshold=0.05; % image motion threshold
for gh=[1 2]
...
print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'LFP_sptrig_pow_DELTA.pdf'])
...
print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'LFP_sptrig_pow_REG.pdf'])
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
errorbar([5   ],nanmean(V4,1), nanstd(V4)./sqrt(size(V4,1)),'.k');
 %print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'spLFP_beta_BARQUANT.pdf'])
...
%df=36, tst=2.7,      0.0260
   [h,p,ci,stats] =ttest2(V3,V4)
%df=23, tst=3.8,      0.0372
   [h,p,ci,stats] =ttest2(V1,V3)
%df=29, tst=3.8,        0.8028
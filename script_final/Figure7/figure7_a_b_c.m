...
for gh=1:2
    ...
    for sesd= [chs]
        ...
        if length(aligned.trace)>0
            ...
            cfg.width =5;
            cfg.t_ftimwin =[ones(1,length(cfg.foi))*0.6];
            ...
            for id=1:size(  freq2.fourierspctrm,1)
                ...
                s= lfp.trial{id}(2,:)>0;s=find(s);s2= lfp.trial{id}(2,:)>0
            ...
            if sum(~isnan(aM(1,101,:)))>10  & sum(~isnan(aM2(1,101,:)))>10
                nn=nn+1;
...
nr=nr+1;
id=wind+1;
allCOH(nr)= nanmean(nanmean((allC(nr,id, selC)),3),2);
allCOHs(nr)= nanmean(nanstd((allC(nr,id, selC)),[],3),2)./sqrt(size(allC(:,:, selC),3));
allCOH2(nr)= nanmean(nanmean((allC2(nr,id, selC)),3),2);
allCOH2s(nr)= nanmean(nanstd((allC2(nr,id, selC)),[],3),2)./sqrt(size(allC2(:,:, selC),3));
end
%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
id=101
...
clear allstT
for b1=1:size(allC,1)
[h,p,ci,stats] =ttest(squeeze(allC(b1,101,:)), squeeze(allC2(b1,101,:)));
...
%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 fsel=find(round(freq2.freq)>=1  & round(freq2.freq)<=4);
 selC=cell_id==1&d_id==1;
V1= squeeze(nanmean(nanmean((allC2( fsel,id, selC)),2),1))- squeeze(nanmean(nanmean((allC( fsel,id, selC)),2),1));
...
%df=35, tst=2.7,           0.0289

   [h,p,ci,stats] =ttest2(V3,V4)
%df=23, tst=3.8,        0.5009


   [h,p,ci,stats] =ttest2(V1,V3)
%df=28, tst=3.8,             0.2325
